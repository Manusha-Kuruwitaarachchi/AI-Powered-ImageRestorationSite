...
symbol_value ={ #the value of each letter
    "A": 5,
    "B": 4,
    "C": 3,
    "D": 2   
...
def get_slot_machine_spin(rows,cols,symbols):
    ...
    for _ in range(cols):
        column = []
        current_symbols = all_symbols[:] #':' copy the list all_symbols
        for _ in range(rows):
...
def spin(balance):
    ...
    while True:
        bet = get_bet()
        total_bet = bet * lines
        short_amount = total_bet - balance
        if(total_bet > balance):
            print(f"U don't have enough t0 bet that amount, your current balance is ${balance}.")
            print(f"Your are $",short_amount,"short")
        else:
            break
